...
AlarmClock.prototype.ListAlarms = async function () {
  return new Promise((resolve, reject) => {
    this._request('ListAlarms', {})
      .then(async result => {
        let alarms = await Helpers.ParseXml(result.CurrentAlarmList)
        if (alarms === undefined) alarms = []
        else if (!Array.isArray(alarms)) alarms = [alarms]
        delete result.CurrentAlarmList
        result.CurrentAlarmList = alarms
        resolve(result)
      })
      .catch(reject)
  })
 ...
 * @returns {Object} parsed response object
 */
AlarmClock.prototype.PatchAlarm = function (id, options) {
  this.debug('AlarmClock.PatchAlarm(%j %j)', id, options)
  return new Promise((resolve, reject) => {
    this.ListAlarms()
      .then(result => {
        let targetAlarm = null
        for (let alarm of result.CurrentAlarmList) {
          if (alarm.ID === id) {
            targetAlarm = alarm
            break
          }
        }
        if (!targetAlarm) reject(new Error('Alarm with id %s not found', id))
        else {
          ...
          this.UpdateAlarm(targetAlarm).then(resolve).catch(reject)
        }
      })
      .catch(reject)
  })
