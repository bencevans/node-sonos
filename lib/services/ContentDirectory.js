 ...
 */
  ...
  var parseResult = async function (input) {
    return await Helpers.ParseXml(input.Result)
  }

  var enumItems = function (resultcontainer) {
    let items = []
  ...
  ContentDirectory.prototype.GetItemResult = async function (options) {
    return new Promise((resolve, reject) => {
      this.Browse(options)
        .then(async data => {
          let result = await parseResult(data)
          let items = enumItems(result['DIDL-Lite'].item)
          resolve({
  ...
  ContentDirectory.prototype.GetContainerResult = async function (options) {
    return new Promise((resolve, reject) => {
      this.Browse(options)
        .then(async data => {
          let result = await parseResult(data)
          let items = enumItems(result['DIDL-Lite'].container)
          resolve({
            returned: data.NumberReturned,
            total: data.TotalMatches,
            items: items
