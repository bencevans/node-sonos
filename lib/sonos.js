...
var generateOptionsFromUri = function (uri, title, region) {
  ...
  var meta = '<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/" xmlns:r="urn:schemas-rinconnetworks-com:metadata-1-0/" xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/"><item id="##SPOTIFYURI##" ##PARENTID##restricted="true"><dc:title>##RESOURCETITLE##</dc:title><upnp:class>##SPOTIFYTYPE##</upnp:class><desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">SA_RINCON' + region + '_X_#Svc' + region + '-0-Token</desc></item></DIDL-Lite>'
  if (uri.startsWith('spotify:track:')) { // Just one track
    ...
    return {
      uri: 'x-sonosapi-radio:' + spotifyUri,
      metadata: meta.replace('##SPOTIFYURI##', '100c206c' + spotifyUri)
        .replace('##RESOURCETITLE##', radioTitle)
        .replace('##SPOTIFYTYPE##', 'object.item.audioItem.audioBroadcast.#artistRadio') // .replace('##SPOTIFYTYPE##', 'object.item.audioItem.audioBroadcast.#artistRadio')
...
/**
 * Add a song from spotify to the queue, just a shortcut to sonos.queue('spotify:track:....')
 * @param  {String}   trackId      The spotify track ID
 * @param  {Function} callback (err, success)
 */
Sonos.prototype.addSpotifyTrack = function (trackId, callback) {
  var uri = 'spotify:track:' + trackId
  this.queue(uri, callback)
}

/**
 * Add a album from spotify to the queue, just a shortcut to sonos.queue('spotify:album:....')
 * @param  {String}   albumId      The spotify album ID
 * @param  {Function} callback (err, success)
 */
Sonos.prototype.addSpotifyAlbum = function (albumId, callback) {
...
Sonos.prototype.playSpotifyRadio = function (artistId, artistName, callback) {
  ...
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#SetAVTransportURI"'
  var body = '<u:SetAVTransportURI xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><CurrentURI>' + options.uri + '?sid=9&amp;flags=8300&amp;sn=7</CurrentURI><CurrentURIMetaData>' + htmlEntities(options.metadata) + '</CurrentURIMetaData></u:SetAVTransportURI>'
  return this.request(this.options.endpoints.transport, action, body, 'u:SetAVTransportURIResponse', function (err, data) {
    if (err) return callback(err)
    if (data[0].$['xmlns:u'] === 'urn:schemas-upnp-org:service:AVTransport:1') {
      return self.play(callback)
