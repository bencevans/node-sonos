...
Sonos.prototype.searchMusicLibrary = async function (searchType, searchTerm, options) {
  ...
  //     .then(data => {
  //       Helpers.ParseXml(data.Result)
  //         .then(didl => {
  //           debug('Parsed didl %j',didl)
  //           let resultcontainer = opensearch ? didl['DIDL-Lite'].container : didl['DIDL-Lite'].item
  //           let items = []
  //           resultcontainer.forEach(item => {
  //             let albumArtUrl = item['upnp:albumArtURI'] || null

  //             if(albumArtUrl && !albumArtUrl.startsWith('http')) albumArtUrl = 'http://' + this.host + ':' + this.port + albumArtUrl
...
Sonos.prototype.getCurrentState = function () {
  ...
  return new Promise((resolve, reject) => {
    ...
    this.request(this.options.endpoints.transport, action, body, 'u:GetTransportInfoResponse')
      .then(result => {
        let state = null
