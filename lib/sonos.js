...
Sonos.prototype.request = function (endpoint, action, body, responseTag, callback) {
  ...
  request({
    ...
    body: Helpers.CreateSoapEnvelop(body)
...
Sonos.prototype.playWithoutQueue = function (uri, callback) {
  ...
  if (options.uri) {
    ...
    var body = '<u:SetAVTransportURI xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><CurrentURI>' + Helpers.EncodeXml(options.uri) + '</CurrentURI><CurrentURIMetaData>' + Helpers.EncodeXml(options.metadata) + '</CurrentURIMetaData></u:SetAVTransportURI>'
...
Sonos.prototype.queue = function (uri, positionInQueue, callback) {
  ...
  var body = '<u:AddURIToQueue xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><EnqueuedURI>' + Helpers.EncodeXml(options.uri) + '</EnqueuedURI><EnqueuedURIMetaData>' + options.metadata + '</EnqueuedURIMetaData><DesiredFirstTrackNumberEnqueued>' + positionInQueue + '</DesiredFirstTrackNumberEnqueued><EnqueueAsNext>1</EnqueueAsNext></u:AddURIToQueue>'
...
module.exports.DeviceDiscovery = require('./deviceDiscovery')
