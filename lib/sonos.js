 ...
 */
Sonos.prototype.play = async function (uri) {
  debug('Sonos.play(%j)', uri)
  return new Promise((resolve,reject)=> {
    if(!uri) { // No Uri means, just start playing
...
Sonos.prototype.setAVTransportURI = async function (options) {
  debug('Sonos.setAVTransportURI(%j)', uri)
  return new Promise((resolve,reject) => {
    ...
    if (typeof options === 'string') options = Helpers.GenerateMetadata(uri, undefined, this.options.spotify.region)
...
Sonos.prototype.selectTrack = async function (trackNr) {
  return new Promise((resolve,reject) => {
    ...
    this.request(this.options.endpoints.transport, action, body, 'u:SeekResponse')
      .then(result => {
        resolve(data['xmlns:u'] === 'urn:schemas-upnp-org:service:AVTransport:1')
      })
      .catch(reject)
  })
