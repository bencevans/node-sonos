 ...
 * @param  {Boolean} coordinatorOnly Only look of the coordinator (usefull for joined devices, eg. stereo pairs)
 * @returns {Boolean}
 */
Sonos.prototype.joinGroup = async function (otherDeviceName, coordinatorOnly = false) {
  debug('Sonos.joinGroup(%s, %s)', otherDeviceName, coordinatorOnly)

  return this.getTopology()
    .then(result => {
      const zoneInfo = coordinatorOnly
        ? result.zones.find(z => z.name.toLowerCase() === otherDeviceName.toLowerCase() && z.coordinator === 'true')
        : result.zones.find(z => z.name.toLowerCase() === otherDeviceName.toLowerCase())
      if (!zoneInfo) {
