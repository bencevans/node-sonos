  ...
  return new Promise((resolve, reject) => {
    ...
    .then(result => { // Do something with the response (that is converted to an object)
      if (!result || !result['s:Envelope']) {
        ...
        reject(new Error(result['s:Envelope']['s:Body']['s:Fault']))
      } else {
...
Sonos.prototype.getVolume = async function () {
  ...
  return this.request(this.options.endpoints.rendering, action, body, responseTag)
    .then(result => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.uri) {
      ...
      this.request(this.options.endpoints.transport, action, body, 'u:SetAVTransportURIResponse')
        .then(result => {
          if ((!options.onlySetUri || options.onlySetUri !== true) &&
            result['xmlns:u'] === 'urn:schemas-upnp-org:service:AVTransport:1') {
            this.play().then(resolve).catch(reject)
          } else {
            resolve(true)
          }
...
Sonos.prototype.avTransportService = function () {
  return new Services.AVTransport(this.host)
}
