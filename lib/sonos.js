...
/**
 * Select specific track in queue
 * @param  {Number}   track_nr    Number of track in queue (optional, indexed from 1)
 * @param  {Function} callback (err, seeked)
 */
Sonos.prototype.selectTrack = function (track_nr, callback) {
  if (typeof track_nr === 'function') {
    callback = track_nr;
    track_nr = 1;
  }
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#Seek"';
  var body = '<u:Seek xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><Unit>TRACK_NR</Unit><Target>' + track_nr + '</Target></u:Seek>';
