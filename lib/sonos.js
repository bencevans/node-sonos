 ...
 * @return {Void}
 */
 Sonos.prototype.currentVolume = function(callback) {
  debug('Sonos.currentVolume(' + ((callback) ? 'callback' : '') + ')');
 ...
 * @return {Void}
 */
 Sonos.prototype.currentMuted = function(callback) {
  debug('Sonos.currentMuted(' + ((callback) ? 'callback' : '') + ')');
  ...
  return this.request(RENDERING_ENDPOINT, action, body, responseTag, function(err, data) {
    ...
    callback(null, parseInt(data[0].CurrentMute[0], 10) ? 'on' : 'off');
  });
};
 ...
 */
 Sonos.prototype.getZoneInfo = function(callback) {
  ...
  this.request(DEVICE_ENDPOINT, action, body, 'u:GetZoneInfoResponse', function(err, data) {
    if(err) return callback(err, data);
    var output = {
      SerialNumber: data[0].SerialNumber[0],
      SoftwareVersion: data[0].SoftwareVersion[0],
      DisplaySoftwareVersion: data[0].DisplaySoftwareVersion[0],
      HardwareVersion: data[0].HardwareVersion[0],
      IPAddress: data[0].IPAddress[0],
      MACAddress: data[0].MACAddress[0],
      CopyrightInfo: data[0].CopyrightInfo[0],
      ExtraInfo: data[0].ExtraInfo[0],
      HTAudioIn: data[0].HTAudioIn[0]
    };
