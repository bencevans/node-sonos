...
Sonos.prototype.currentTrack = function (callback) {
  ...
  return this.request(this.options.endpoints.transport, action, body, responseTag, function (err, data) {
    ...
    var queuePosition = parseInt(data[0].Track[0], 10)
 ...
 * @param  {Function} callback (err, queued)
 */
Sonos.prototype.queueUpNext = function (uri, callback) {
  var self = this

  this.currentTrack(function (err, track) {
    if (err) {
      callback(err)
    }
    var position = track.queuePosition + 1
    self.queue(uri, position, callback)
  })
}
