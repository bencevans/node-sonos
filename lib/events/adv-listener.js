...
const ParseEvent = require('./eventParser').ParseEvent
...
class SonosListener extends EventEmitter {
  ...
  async stopListener () {
    if (this._listening) {
      ...
      return new Promise((resolve, reject) => { reject('Not listening') })
    }
  }
        ...
        parseXml(body)
          .then(parsedBody => {
            ...
            if (deviceEventEndpoints.indexOf(endpoint) > -1) {
              ...
              self.handleGlobalNotification(endpoint, eventBody)
            }
            resp.statuscode = 200
            resp.end('OK')
          })
  ...
  handleGlobalNotification (endpoint, body) {
...
class DeviceSubscription {
  ...
  async handleNotification (endpoint, body) {
    ...
    let sonosEvent = await ParseEvent(endpoint, body, this.device)
