...
EventParser._parseAvTransportEvent = async function (body, device) {
  ...
  if (eventData['r:NextTrackMetaData']) {
    let nextMeta = await Helpers.ParseXml(eventData['r:NextTrackMetaData'])
    let uri = eventData['r:NextTrackURI'] || null
...
EventParser._parseZoneGroupTopologyEvent = async function (body, device) {
  ...
  if (eventData.ZoneGroupState) {
    console.log(eventData.ZoneGroupState.ZoneGroups)
    eventData.Zones = eventData.ZoneGroupState.ZoneGroups.ZoneGroup.map(zone => { return new SonosGroup(zone) })
  }
