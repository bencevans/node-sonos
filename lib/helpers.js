...
Helpers.GenerateMetadata = function (uri, title = '', region = '3079') {
  var parts = uri.split(':')
  var sid = url.parse(uri, true).query.sid;

  if (!((parts.length === 2 && (parts[0] === 'radio' || parts[0] === 'x-sonosapi-stream' || parts[0] === 'x-rincon-cpcontainer')) || (parts.length >= 3 && parts[0] === 'spotify') || sid == "151")) {
  ...
  if (uri.startsWith('spotify:track:')) { // Just one track
  ...
  } else if (sid == "151") { // Google Play Music
    const [id] = uri.match(/^x-rincon-cpcontainer:(.*)\?.*/).splice(1)
    ...
    return {
        ...
        metadata: meta.replace('##SPOTIFYURI##', id)
            ...
            .replace('##SPOTIFYTYPE##', 'object.container.playlistContainer')
            .replace('##PARENTID##', '')
            .replace('##REGION##', 'SA_RINCON38663_X_#Svc38663-0-Token')
    }
  } else {
    return { uri: uri, metadata: '' }
  }
